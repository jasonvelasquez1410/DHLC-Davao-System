...
const CloudDrive = () => {
  ...
  const handleOpenPicker = () => {
    try {
                    ...
                    const picker = new window.google.picker.PickerBuilder()
                        ...
                        .setCallback((data) => {
                            if (data.action === window.google.picker.Action.PICKED) {
                                ...
                                // Here we can save the doc.url to Firebase
                            }
                        })
                        .build();
        ...
        alert("Google services are still loading. Please wait 2 seconds and try again.");
    }
  };
  ...
  const resources = [
    ...
    {
      id: 'f3',
      name: 'Q3 Financial Accountability.xlsx',
      type: 'xlsx',
      category: 'internal',
      owner: 'Finance Dept',
      date: 'Oct 15',
      ...
      <div className="container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
        ...
        {selectedFile && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 10000, display: 'flex', flexDirection: 'column', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
               ...
               <button onClick={() => setSelectedFile(null)} className="btn-ghost" style={{ padding: '10px' }}>
                 <X size={24} />
               </button>
            </div>
        ...
        <div className="animate-fade-in" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '3rem', flexWrap: 'wrap', gap: '1.5rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-dim)', marginTop: '0.5rem' }}>Cloud-synced ministry resources for the DHLC global community.</p>
          </div>

          <div style={{ display: 'flex', gap: '1rem' }}>
            {isLeader && (
              <>
                <button className="btn-ghost" style={{ padding: '0.8rem 1.5rem' }} onClick={handleOpenPicker}>
                   <HardDrive size={18} /> Import from Drive
                </button>
          ...
          <div className="animate-fade-in">
                   ...
                   {['all', 'sermons', 'resources', 'internal'].map(cat => (
                     <button 
                       ...
                       style={{ 
                         ...
                         padding: '12px 15px', 
                         borderRadius: '10px', 
                         background: activeCategory === cat ? 'rgba(242,153,0,0.1)' : 'transparent',
             ...
             <div className="premium-card" style={{ padding: '1.5rem', background: 'linear-gradient(135deg, rgba(242,153,0,0.05), transparent)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
                   ...
                   <h4 style={{ margin: 0 }}>Workspace Link</h4>
                </div>
                <p style={{ fontSize: '0.85rem', color: 'var(--text-dim)', lineHeight: '1.5' }}>
                  Your account is verified via Google Workspace. You have seamless access to ministry-restricted documents.
                </p>
                <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                   ...
                   <span style={{ fontSize: '0.75rem', fontWeight: 'bold' }}>VERIFIED ACCOUNT</span>
                </div>
             </div>
          </div>
                        ...
                        <tr key={res.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', transition: 'background 0.2s' }}>
                           <td style={{ padding: '1.2rem 1.5rem' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                 {res.type === 'docx' ? <FileText size={20} color="#2b579a" /> : 
                                  res.type === 'xlsx' ? <Share2 size={20} color="#217346" /> : 
                           ...
                           <td style={{ fontSize: '0.85rem', color: 'var(--text-dim)' }}>{res.date}</td>
                           <td style={{ textAlign: 'right', paddingRight: '1.5rem' }}>
                              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                                 ...
                                 <a href={res.url} download className="btn-ghost" style={{ padding: '8px', borderRadius: '50%' }}><Download size={18} /></a>
                              </div>
                           </td>
                        </tr>
                      ))}
